...
def local_backup(
    ...
    logger.info(f"start copy:\n    {remote_host}:{remote_filepath}\n    to: local {local_filepath}")
    try:
        ssh_client = SSHClient()
        ssh_client.load_system_host_keys()
        ssh_client.set_missing_host_key_policy(AutoAddPolicy())
        ssh_client.connect(
            remote_host, username=remote_username, password=remote_password, port=remote_port
        )
        
        with SCPClient(ssh_client.get_transport()) as scp_client:
            scp_client.get(remote_filepath, local_filepath)
    except Exception as e:
        raise Exception(e) # still want exception to be raised - might rely on it outside...
    finally:
        # Make sure that 
        if ssh_client:
            ssh_client.close()
...
def backup_and_add_header_keys(
    ...
    **header_keys
    ...
    with fits.open(local_raw_filepath) as f:
        ...
        f[0].header["DFAPRNM"] = (dfosc_status.aper_name_guess, "DFOSC aperture name")
        f[0].header["DFAPRPOS"] = (dfosc_status.aper_position, "DFOSC aperture position")
        f[0].header["DFGRINM"] = (dfosc_status.grism_name_guess, "DFOSC grism name")
        f[0].header["DFGRIPOS"] = (dfosc_status.grism_position, "DFOSC grism postion")
        f[0].header["DFFLTNM"] = (dfosc_status.filter_name_guess, "DFOSC filter name")
        f[0].header["DFFLTPOS"] = (dfosc_status.filter_position, "DFOSC filter position")
