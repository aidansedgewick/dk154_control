...
slits = yaml.load(open('dk154_control/dfosc/dfosc_setup.yaml'), Loader=yaml.FullLoader)['slit']
filters = yaml.load(open('dk154_control/dfosc/dfosc_setup.yaml'), Loader=yaml.FullLoader)['filter']
...
if __name__ == '__main__':
	...
	#parser.add_argument('time', type=str, help='Time of the day (morning or evening)')
	parser.add_argument('-n', type=int, help='Number of frames to take')
	parser.add_argument('-g', type=str, help='Grism number. Valid grisms are empty,3,5,6,7,8,14,15') #TODO: update options when .yaml is changed
	parser.add_argument('-s', type=str, help='Slit size. Valid slits are empty,1,1.5,2,2.5,3.0,pinhole,5.0')
	parser.add_argument('-f', type=str, help='Filter type. Current filters are empty0,empty1,empty2,empty3,10,11,12,empty4') 
