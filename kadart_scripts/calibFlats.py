...
# Script for taking flat fields
# Daniel Evans 2017-09-14
# For normal filters, the usual commands are:
# ~/scripts/takeFlats.py evening Halpha U B V R I
# ~/scripts/takeFlats.py morning I R V B U Halpha
...
import glob
import numpy
import pyfits # no astropy available
import telOffset

# The names and positions of the filters in each wheel
filtersA = ["empty", "Stromgr u", "Stromgr v", "Stromgr b", "Stromgr y", 
	"H-beta", "g2", "Gunn z"]
filtersB = ["empty", "U", "B", "V", "R", "I", "Halpha"]
...
# The sky brightness is assumed to be an exponential decay 
# (or growth) with time, of the form A exp(tau * t)
# The following values are fitted values for tau, derived from
# previously taken flat fields.
# More obscure filters don't have any reference, so I've
# given a default constant near the average of the others.
# Since the script updates the exposure times based on the
# previous exposure anyway, a slightly wrong constant won't
# do much harm.
...
def parseFilters(filterList):
	...
	for filterName in filterList:
		...
		if filterName in filtersA:
			idx = filtersA.index(filterName)
			entry = [idx, 0]
		elif filterName in filtersB:
			idx = filtersB.index(filterName)
			entry = [0, idx]
		else:
...
if __name__ == "__main__":
	...
	for arg in args:
		...
		if argLowercase == "morning":
			...
			isMorning = False
			print("Evening")
		elif arg in filtersA or arg in filtersB:
